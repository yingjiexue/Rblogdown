...
输出word的数学公式按照markdown内公式编辑的方式即可，具体命令参考：[Markdown 中的公式命令](https://rpruim.github.io/s341/S19/from-class/MathinRmd.html)。这儿需要提示的是公式编号，
利用markdown输出数学公式后并带公式编号，要实现公式自动编号，需要在yaml中加入pandoc命令：‘pandoc_args: ["-Fpandoc-crossref"]’，然后在"$$"符号后面添加标签："{#eq:eqn2}",示例如下：
...
在R markdown中引用参考文献，比word方便了很多倍，我们可以借助Zotero来管理文献，个人觉得它比Endnote好用许多，优点在于其下载文献方便，还可以做笔记，最重要的一点是可以导出markdown可以应用的.bib格式的文件，当你使用R markdown写论文时，可以从Zotero中导出论文中要引用的文献成.bib格式，然后复制到markdown根目录，在yaml中添加该文件名就可以自由引用了。更方便的是，Zotero文献引用格式库几乎提供了所有文献引用格式，你将对应文献引用格式.csl文件下载，与.bib格式文件放在相同目录下，同样在yaml中配置参数后就可以按你想要的引用格式引用参考文献了。yaml配置参数如下：
...
> Zotero 如何使用你可以找B站，直接在下面看。

knitr::include_url("https://www.bilibili.com/video/BV1eq4y137hM?spm_id_from=333.337.search-card.all.click&vd_source=1312148eed03d2915d234e39fdc4c179")