...
>
> 如图1所示，通过绘制因子收益和因子溢价的累积总和来估计中断或趋势。该框架允许灵活地捕获不同频率的中断，或者换句话说，不同平滑度的趋势，高正则化强度产生更平滑的趋势(如图1中的蓝色趋势线所示)。在极低的正则化条件下，模型简化为逐周期的横截面回归，$\gamma_t$ 估计接近斜率因子本身(图1中红色锯齿状趋势线)。因此，TV
...
> 其中， `\(r_{i,t+1}\)` 是股票 `\(i\)` 在 `\(t\)` 时期的收益，  `\(z_{i,t}\)` 是一个外生的回归变量， `\(f_{t+1}=\gamma_t+v_{t+1}\)`
